...
Upon receiving the Complete Multipart Upload request, XDM   concatenates all
...
minutes to complete. Once XDM   begins processing the request, it sends an
HTTP response header that specifies a ``200 OK`` response. While
processing is in progress, XDM   periodically sends whitespace characters to
