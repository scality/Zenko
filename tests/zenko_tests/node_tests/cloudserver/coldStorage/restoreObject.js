...
// const { getS3Client, iam } = require('../../s3SDK');
// const { getSTSClient } = require('../../stsSDK');
//
// const { getTokenForIdentity } = require('../../utils/getWebIdentityToken');
// const { restoreObjectResponseCode, createPolicy } = require('../../iam_policies/cloudserver/utils');
//
// let iamClient = null;
// let stsClient = null;
// let s3Client = null;
...
//
// describe.skip('cold storage - cloudserver - AssumeRoleWithWebIdentity - RestoreObject', () => {
...
//                 next();
//             },
//             next => iamClient.createUser({ UserName: userName }, next),
//             next => iamClient.createAccessKey({ UserName: userName }, next),
...
//                     restoreObjectResponseCode(sessionUserCredentials, bucket1, err => {
//                         if (err) {
//                             assert.ifError(err);
//                             return done(err);
//                         }
...
//         return restoreObjectResponseCode(userCredentials, bucket1, err => {
//             if (err) {
//                 assert.ifError(err);
//                 return done(err);
//             }
...
//     });
//
//     it('should be able to metadata search with the permission s3:RestoreObject allowed', done => {
//         const policyArns = [];
//         async.series(
//             [
//                 next => iam.createPolicy({
//                     PolicyName: 'allowRestoreObjectPolicyName',
...
//                 }),
//                 next => restoreObjectResponseCode(userCredentials, bucket1, (err, res) => {
//                     if (err) {
//                         assert.ifError(err);
//                         return next(err);
//                     }
