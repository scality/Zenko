...
describe('OOB updates for RING S3C bucket', () => {
    ...
    it('should receive OOB update with an object', done => async.waterfall([
    ...
    it('should receive OOB update with 0-byte object', done => async.waterfall([
        next => ringS3CUtils.putObject(
            ingestionSrcBucket,
            OBJ_KEY,
            null,
    ...
    it('should receive OOB update with tagged object', done => async.waterfall([
        next => ringS3CUtils.putObject(
            ingestionSrcBucket,
            OBJ_KEY,
            null,
        ...
        (tagObjData, next) => scalityUtils.compareObjectTagsRINGS3C(
            ingestionSrcBucket,
            INGESTION_DEST_BUCKET,
            OBJ_KEY,
            tagObjData.VersionId,
            next,
        ),
    ], done));

    it('should receive OOB update with all versions of an object', done => async.waterfall([
        next => ringS3CUtils.putObject(
            ingestionSrcBucket,
            OBJ_KEY,
            null,
    ...
    it('should receive OOB update with MPU object: single 0-byte part', done => async.waterfall([
    ...
    it('should receive OOB updated with MPU object: single 1-byte part', done => async.waterfall([
