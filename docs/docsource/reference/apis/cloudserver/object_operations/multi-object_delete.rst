...
each key, |product| performs a delete operation and returns the result of that
delete, success or failure, in the response. If the object specified in the
request is not found, |product| returns the result as deleted.
...
requests. |product| uses the header value to ensure that your request body has not
...
.. table::
   ...
   |                           |           | |product| attempted to delete     |
   |                           |           |                                   |
   |                           |           | **Ancestor:** Deleted, Error      |
   +---------------------------+-----------+-----------------------------------+
   | ``VersionId``             | String    | Version ID of the versioned       |
   |                           |           | object |product| attempted to     |
   |                           |           | delete. Includes this element     |
   |                           |           | only in case of a versioned-delete|
   |                           |           | request.                          |
   |                           |           |                                   |
   ...
   |                           |           |    case, |product| creates a      |
   |                           |           |    delete marker and returns its  |
   ...
   |                           |           |    marker. In this case,          |
   |                           |           |    |product| deletes the delete   |
   |                           |           |    marker and responds with the   |
   |                           |           |    specific version ID.           |
   |                           |           |                                   |
   |                           |           | **Ancestor:** Deleted             |
   +---------------------------+-----------+-----------------------------------+
   ...
   |                           |           | object that |product| attempted to|
   ...
   |                           |           | Code, Message                     |
   +---------------------------+-----------+-----------------------------------+
   | ``Key``                   | String    | Key for the object |product|      |
   |                           |           | attempted to delete               |
   |                           |           |                                   |
   |                           |           | **Ancestor:** Error               |
   +---------------------------+-----------+-----------------------------------+
...
the item that |product| successfully deleted and an Error element that |product|  did not
...
object remain in the bucket; however, |product| inserts a delete marker.
...
Because versioning is enabled on the bucket, |product| does not delete the object,
...
In this case, |product| deletes the specific object version from the bucket and
returns the following response. In the response, |product| returns the key and
...
of an object), then |product| deletes the delete marker. The effect of this
operation is to make the object reappear in the bucket. The response
returned by |product| indicates the deleted delete marker (DeleteMarker element
...
In general, when a Multi-Object Delete request results in |product| either adding
