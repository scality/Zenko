...
function getAndCheckResponse(path, expectedBody, cb) {
    ...
    return doWhilst(
        ...
        () => shouldContinue && !isPendingOk,
        err => {
            if (err) {
                return cb(err);
            }
            if (isPendingOk === false) {
...
describe('Backbeat object monitor CRR metrics', () => {
    ...
    it('should monitor a 1 byte object', done => waterfall([
        ...
        (data, next) => scalityUtils.waitUntilReplicated(srcBucket, key, undefined, err => next(err, data)),
        (data, next) => {
            ...
            return doWhilst(
                ...
                err => {
                    ...
                    const hasInterval = responses.some(response => {
                        ...
                        const percentage = Number.parseInt(response.progress.split('%')[0], 10);
                        const hasInterval = (pending > 0 && pending < contentLength)
    ...
    it('should monitor the average throughput for a 10 byte object', done => {
        ...
        return waterfall([
            ...
            (data, next) => scalityUtils.waitUntilReplicated(
                srcBucket,
                throughputKey,
                undefined,
                err => next(err, data),
