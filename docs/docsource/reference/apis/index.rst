...
S3 Connector comprises several component REST APIs. Many of the endpoint
...
.. toctree::
   ...
   cloudserver_api/index
   backbeat_api/index
   metadata_api/index
   utapi/index
   vault_api_reference/index
...
S3 Connector's encryption scheme is architected around bucket-level
...
S3 Connector's Key Management Service (KMS) integration for bucket encryption is
...
Connector and the RING do not store encryption keys. Currently, S3 Connector is
integrated with one KMS solution, Gemalto SafeNet KeySecure.

S3 Connector uses standard OpenSSL, 256-bit encryption libraries to perform the
...
In version 7.6, S3 Connector is modified to accept object encryption headers.
Object-level encryption is not supported; however, S3 Connector no longer throws
...
If S3 Connector encounters an object-level encryption header and bucket-level
