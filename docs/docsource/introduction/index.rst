...
|product| manages the complexities and discontinuities between the cloud storage
protocols and services described in :ref:`Supported Clouds and Services`, and
manages a unified namespace to store and retrieve data seamlessly from all its
cloud locations. To do this, |product| maintains its own namespace, which it
uses to manage and track objects in other namespaces. For example, a cloud might
spread a bucket's contents over many servers. |product| trusts this cloud to
work as designed and provide the bucket, or individual objects in the bucket, on
demand. |product| knows what the source cloud calls the data, and if it should
replicate it to a target cloud, it again relies on the target cloud to maintain
the data fed to it: |product| only deals with cloud storage systems with the
APIs they provide. In effect, |product| operates by maintaining a namespace of
namespaces.
