...
XDM, Scality's multi-cloud controller, provides an open-source,
platform-agnostic gateway to facilitate data replication and management for
storage managers handling extreme data volumes over multiple clouds. XDM  
provides a single integration point from which cloud data can be managed in
several protocol spaces. XDM   either builds a namespace for cloud object data
...
XDM   offers these capabilities using the logic and much of the syntax of
...
With Release 1.2, XDM   introduced an Azure Blob-native storage API, Blobserver,
...
Most XDM   tasks can be managed using the web-based Orbit service. More advanced
users, however, may wish to interact directly with XDM   using its REST
...
XDM   provides application programming interfaces via CloudServer, UTAPI,
and Prometheus.

Command XDM   by calling its REST API, using:
