...
subtree merge**. Future upgrades merge the new upstream via
`git subtree merge --squash`, which produces real three-way merge
conflicts where upstream has touched lines our local commits modified.
...
7. After upgrading, you may need to apply additional changes that are
   not directly handled by the chart. Carefully review the new logic
   added by the new chart version and ensure it remains compatible with
   our use case.

8. When bumping the Helm chart, also check whether the image Dockerfile
   or Bitnami scripts changed for that chart version. If so, re-vendor
   the scripts from `bitnami/containers` and rebuild the images before
   deploying the new chart (see `solution-base/images/README.md`).

9. Passing CI tests is not enough — perform an upgrade check against
