...
Upon receiving the Complete Multipart Upload request, |product| concatenates all
...
minutes to complete. Once |product| begins processing the request, it sends an
HTTP response header that specifies a ``200 OK`` response. While
processing is in progress, |product| periodically sends whitespace characters to
