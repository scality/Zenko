...
(this: Zenko, bucketName: string, withVersioning: string, withObjectLock: string, retentionMode: string) {
    ...
    this.getSaved().bucketName = usedBucketName;
...
Given('an object {string} that {string}',
    async function (this: Zenko, objectName: string, objectExists: string) {
        ...
        if (objectExists === 'exists') {
            this.getSaved().objectName = objectName || Utils.randomString();
            this.addCommandParameter({ key: this.getSaved().objectName });
            this.addCommandParameter({ bucket: this.getSaved().bucketName });
            this.getSaved().versionId = extractPropertyFromResults(
