...
Once a XDM instance is up and running, you can upgrade it with a
simple Helm command. 
...
To see the custom values of a running XDM instance::
...
.. note::
   ...
   their upgrade implications must be considered by both the XDM and
...
#. Back up your existing XDM directory.
...
#. Copy Zenko/kubernetes/options.yaml from your existing Zenko
   source directory to the same directory in the new XDM source.  
...
#. From the kubernetes/ directory of the new XDM source, enter this
   Helm command, inserting your XDM server's name:
