...
let srcBucket = `source-bucket-${Date.now()}`;
let destBucket = process.env.AWS_CRR_BUCKET_NAME;
...
describe('Replication with AWS backend', function () {
    ...
    beforeEach(done => series([
        ...
        next => {
            srcBucket = `source-bucket-${Date.now()}`;
            return next();
        }
    ], done));
    ...
    [undefined,
        `0-${1024 * 1024 * 5}`,
        `${1024 * 1024 * 2}-${1024 * 1024 * 7}`].forEach(range => it.skip('should replicate a MPU'
        + ' with parts copied from another MPU with '
    ...
    it('should not replicate object ACL', done => series([
        ...
        next => scalityUtils.expectReplicationStatus(
            srcBucket,
            key,
            undefined,
            'COMPLETED',
    ...
    it('should replicate object tags of the latest version', done => series([
        ...
        next => scalityUtils.compareObjectTagsAWS(
            srcBucket,
            destBucket,
            key,
            undefined,
    ...
    it('should replicate object tags of a previous version', done => {
        ...
        return series([
            ...
            next => scalityUtils.putObjectTagging(
                srcBucket,
                key,
                firstVersionScality,
                next,
    ...
    it(
        'should replicate deleting object tags of the latest version',
            ...
            next => scalityUtils.deleteObjectTagging(
                srcBucket,
                key,
                undefined,
                next,
    ...
    it.skip('should replicate object tags of the latest MPU version', done => series([
        next => scalityUtils.completeMPUAWS(srcBucket, key, 2, next),
        next => scalityUtils.compareObjectsAWS(
            srcBucket,
            destBucket,
    ...
    + 'put before replication is complete', done => series([
    ...
    // replicated, so skip it for now
    it.skip('should replicate object tags of an MPU version when tagging is '
    + 'put and then deleted before replication is complete', done => series([
        next => scalityUtils.completeMPUAWS(srcBucket, key, 5, next),
        next => scalityUtils.putObjectTagging(
            srcBucket,
            key,
    ...
    it.skip('should replicate an MPU object with custom user metadata', done => series([
    ...
    it.skip('should replicate an MPU object with content disposition', done => series([
        next => scalityUtils.completeMPUAWSWithProperties(
            srcBucket,
            key,
            Buffer.alloc(1),
    ...
    it.skip('should replicate an MPU object with content encoding', done => series([
        next => scalityUtils.completeMPUAWSWithProperties(
            srcBucket,
            key,
            Buffer.alloc(1),
    ...
    it.skip('should replicate an MPU object with content language', done => series([
        next => scalityUtils.completeMPUAWSWithProperties(
            srcBucket,
            key,
            Buffer.alloc(1),
