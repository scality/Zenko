...
See :version-ref:`XDM Installation
...
The XDM Local filesystem is a convenient, easily configured test location that
enables product familiarization and simple operations. It is internal to XDM,
and serves as the default location for otherwise unnamed locations that rely on
the default "us-east-1" location. Because it is internal to XDM, the "Add New
...
.. warning::
   ...
   While convenient for testing purposes, the XDM Local filesystem is not
   recommended for use in a production setting. The XDM Local filesystem
...
XDM can access information and file system metadata over the NFSv3 and NFSv4
protocols. To configure XDM to access NFS using out-of-band updates, review
...
.. important::
   ...
   replication. For NFS CRR, XDM scans the NFS file system, then detects and
...
XDM can access information and file system metadata from AWS S3 services using
out-of-band updates in a flow similar to that used to extrapolate metadata from
NFS. To configure XDM to access AWS using out-of-band updates, follow the
