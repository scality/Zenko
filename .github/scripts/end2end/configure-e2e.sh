...
kubectl run kafka-topics \
    ...
    --attach=True \
    --command -- bash -c \
    "kafka-topics.sh --create --topic $UUID.backbeat-replication-replay-0 --bootstrap-server $KAFKA_HOST_PORT ; \
    kafka-topics.sh --create --topic $UUID.backbeat-data-mover --bootstrap-server $KAFKA_HOST_PORT ; \
