...
#. Each storage location type has its own requirements. No security is
   ...
   .. note::
      ...
      XDM requests credentials for a location, it is a best practice to enter
      ...
      or an S3 Connector, for example) for the purpose of XDM access. Give
      that XDM "user" all and only the permissions needed to perform the
      desired tasks.
...
All the cloud storage services serviced by XDM require the same basic
...
to name endpoints. Services for which XDM requests endpoint names may have
...
sure it is accessible to XDM (firewall open, etc.).

Bucket Match
~~~~~~~~~~~~

XDM provides a "Bucket Match" option for Ceph RADOS Gateway and Scality S3
Connector. If this option is left unchecked, XDM prepends a bucket identifier
...
large bucket and XDM manages the namespace of the "sub-buckets." Clicking the
...
.. important::
   ...
   used as a CRR destination. XDM requires the bucket identifier in order to
...
protocol), clicking **Server Side Encryption** forces XDM to include
...
chunks, and on a successful upload reassembled in the cloud. XDM manages this
complexity, in part, by using a "helper" bucket.

.. note::

   Google Cloud Storage also imposes a 1024-part cap on objects stored to its
   locations (For all other backends, XDM caps the number of parts at
   10,000). For data stored directly to GCP as the primary cloud, XDM
   propagates this limitation forward to any other cloud storage services to
   which Google data is replicated.

Other Services: XDM Local, RING/sproxyd, and NFS
--------------------------------------------------

XDM Local Filesystem
~~~~~~~~~~~~~~~~~~~~~~

XDM Local Filesystem has similar authentication requirements to AWS S3, but
because it is a XDM-native filesystem, it shares authentication and related
credentialing tasks, which are addressed elsewhere in the Orbit UI.

For more information, see :ref:`XDM Local`.
...
number. If a port number is not explicitly assigned, XDM assigns port 8081 by
default. Entries can use DNS or IP address format.

NFS
~~~

XDM supports out-of-band updates from NFSv3 and NFSv4 file systems. XDM
replicates data from NFS servers to cloud storage services using scheduled cron
jobs.

.. note::

   For NFS mounts, XDM cannot perform data PUT transactions. In other words,
   data can be written directly to NFS for XDM to replicate to other backends,
   but cannot be written to XDM to replicate to NFS.
...
XDM can ingest metadata out of band from AWS in much the same way it can
ingest out-of-band updates from NFS mounts. AWS metadata is ingested in an
initial setup, then changes are mapped via a regularly scheduled cron job. XDM
...
Both RING with sproxyd and XDM Local file systems can be configured
