...
Then('kafka consumed messages should not take too much place on disk', { timeout: -1 },
    async function (this: Zenko) {
        ...
        try {
            // List from Zenko Operator
            const cleanedTopicSuffixes = [
                'backbeat-ingestion',
            ...
            while (remaining.size > 0) {
                ...
                const remainingTopics = [...remaining];
                ...
                for (let i = 0; i < remainingTopics.length; i++) {
                    ...
                    const oldPartitions = previousOffsets.get(topic)!;
                    ...
                    for (let j = 0; j < newOffsets[i].partitions.length; j++) {
                        ...
                        const oldOffsetPartition = oldPartitions[j];
...
Then('kafka cleaner has successfully cleaned topic messages', async function (this: Zenko) {
    const prometheusQuery = encodeURIComponent(
        'kafka_cleaner_kafka_requests{type="DeleteRecords",status="success"}'
    );
