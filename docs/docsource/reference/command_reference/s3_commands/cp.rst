...
customer-provided encryption key for XDM   to use to decrypt the source
...
``--grants`` (string)
  ...
  * ``Grantee_ID`` - Specifies the grantee based on Grantee_Type. The
  ...
  For more information on XDM   access control, see `Access Control
  <http://docs.aws.amazon.com/AmazonS3/latest/dev/UsingAuthAccess.html>`__

``--website-redirect`` (string)

If the bucket is configured as a website, redirects requests for this object to
another object in the same bucket or to an external URL. XDM   stores the
