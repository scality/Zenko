...
Helm installs XDM using packages of Kubernetes resource definitions known as
charts. These charts, which Helm follows for each XDM component, can be found
...
Follow these steps to install XDM with ingress.
...
#. If your XDM instance is behind a proxy, add the following lines to the
   options.yaml file:
...
#. To see Kubernetes's progress creating pods for XDM, the command
   ...
   When all pods stabilize, XDM is installed. 
...
#. To register your XDM instance for Orbit access, get your CloudServer’s
   name::
