...
# Initialize functional test resources (accounts, buckets, locations, etc.)
FUNCTIONAL_TESTS_DIR="$(cd "$DIR/../../../tests/functional" && pwd)"
SETUP_DIR="$(cd "$DIR/../../../tests/functional/testResourcesSetup" && pwd)"
RESOURCES_CONFIG="$SETUP_DIR/resources.yaml"

envsubst < "$SETUP_DIR/resources.yaml.template" > "$RESOURCES_CONFIG"
