...
infrastructure services. XDM uses Burry to back up and restore critical
...
Burry is installed with XDM as a cronjob in suspended mode. To enable it, edit
the options.yaml file; then update XDM with Helm. It is toggled off by default
with the ``suspend:`` parameter set to ``true``. Setting this parameter to ``false``
activates the feature when pushed to a XDM server.
...
XDM server with a Helm command::
