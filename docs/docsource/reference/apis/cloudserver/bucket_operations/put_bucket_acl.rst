...
granting individualized bucket access permissions to specific |product| accounts
or groups. Each of these headers maps to specific permissions the |product|  
supports in an ACL.
...
predefined by |product| and two accounts identified by their email addresses.
...
.. note::
  ...
  permission is not currently being used by |product|.
...
-  Write permission to the |product| LogDelivery group and an account identified
   by ``xyz@example.com``
-  Read permission to the |product| AllUsers group
