...
describe('Replication with AWS backend', () => {
    ...
    test('should replicate a MPU object: single 0 byte part', done => series([
        next => scalityUtils.completeSinglePartMPU(srcBucket, key, 0, next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
            undefined, next),
    ], done));

    test('should replicate a MPU object: single 1 byte part', done => series([
    ...
    test('should not replicate object ACL', done => series([
        next => scalityUtils.putObject(srcBucket, key, Buffer.alloc(1), next),
    ...
    test('should put delete marker on destination bucket when deleting the ' +
    ...
    test('should replicate object tags of the latest version', done => series([
        next => scalityUtils.putObject(srcBucket, key, Buffer.alloc(1), next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
            undefined, next),
    ...
    test('should replicate object tags of a previous version', done => {
    ...
    test('should replicate deleting object tags of a previous version', done => {
        let firstVersionScality = null;
        let firstVersionAWS = null;
        return series([
            next => scalityUtils.putObject(srcBucket, key, Buffer.alloc(1),
    ...
    test('should replicate object tags of the latest MPU version', done =>
    series([
        next => scalityUtils.completeMPUAWS(srcBucket, key, 2, next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
            undefined, next),
    ...
    test.skip('should replicate object tags of an MPU version when tagging is ' +
    ...
    test('should replicate object tags of a previous MPU version', done => {
        let firstVersionScality = null;
        let firstVersionAWS = null;
        return series([
            next => scalityUtils.completeMPUAWS(srcBucket, key, 2, next),
    ...
    test(
        'should replicate deleting object tags of the latest MPU version',
        done => series([
            next => scalityUtils.completeMPUAWS(srcBucket, key, 2, next),
            next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
                undefined, next),
    ...
    test(
        'should replicate deleting object tags of a previous MPU version',
        done => {
            let firstVersionScality = null;
            let firstVersionAWS = null;
            return series([
    ...
    test('should replicate an object with custom user metadata', done =>
    series([
    ...
    test('should replicate an object with content-type', done => series([
    ...
    test('should replicate an object with cache control', done => series([
    ...
    test('should replicate an object with content disposition', done =>
    series([
    ...
    test('should replicate an object with content encoding', done =>
    series([
    ...
    test('should replicate an object with content language', done =>
    series([
    ...
    test('should replicate an object copy with custom user metadata', done =>
    series([
        next => scalityUtils.putObjectWithUserMetadata(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
    ...
    test('should replicate an object copy with content-type', done =>
    series([
        next => scalityUtils.putObjectWithContentType(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.copyObject(srcBucket, copySource, copyKey, next),
    ...
    test('should replicate an object copy with cache control', done =>
    series([
        next => scalityUtils.putObjectWithCacheControl(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.copyObject(srcBucket, copySource, copyKey, next),
    ...
    test('should replicate an object copy with content disposition', done =>
    series([
        next => scalityUtils.putObjectWithContentDisposition(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.copyObject(srcBucket, copySource, copyKey, next),
    ...
    test('should replicate an object copy with content encoding', done =>
    series([
        next => scalityUtils.putObjectWithContentEncoding(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.copyObject(srcBucket, copySource, copyKey, next),
    ...
    test('should replicate an object copy with content language', done =>
    series([
        next => scalityUtils.putObjectWithContentLanguage(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.copyObject(srcBucket, copySource, copyKey, next),
    ...
    test('should replicate an MPU object with custom user metadata', done =>
    series([
    ...
    test('should replicate an MPU object with content-type', done => series([
    ...
    test('should replicate an MPU object with cache control', done => series([
        next => scalityUtils.completeMPUAWSWithProperties(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
            'CacheControl', next),
    ...
    test('should replicate an MPU object with content disposition', done =>
    series([
        next => scalityUtils.completeMPUAWSWithProperties(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
    ...
    test('should replicate an MPU object with content encoding', done =>
    series([
        next => scalityUtils.completeMPUAWSWithProperties(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
    ...
    test('should replicate an MPU object with content language', done =>
    series([
        next => scalityUtils.completeMPUAWSWithProperties(srcBucket, key,
            Buffer.alloc(1), next),
        next => scalityUtils.compareObjectsAWS(srcBucket, destBucket, key,
...
describe('Replication with AWS backend: source AWS location', () => {
    this.timeout(REPLICATION_TIMEOUT);
    let roleArn = 'arn:aws:iam::root:role/s3-replication-role';
