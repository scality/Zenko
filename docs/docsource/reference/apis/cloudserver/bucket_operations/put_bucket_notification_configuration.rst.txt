...
When Zenko receives this request, it verifies that the aws sqs Kafka destination
...
.. note::
   ...
   Zenko supports QueueConfiguration notification types only. There is no
...
.. note::
   ...
   this configuration, Zenko sends test messages to your Kafka queue. If
   the message fails, the entire PUT operation fails, and Zenko does not
...
identifying an SQS queue for Zenko to publish events of the
...
identifying an s3:ObjectCreated:Put type Kafka queue for Zenko to publish
