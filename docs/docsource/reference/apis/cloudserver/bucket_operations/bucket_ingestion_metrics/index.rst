...
XDM   can make queries against XDM    instances named as bucket
locations under its management. Querying the managed namespace, XDM  
can retrieve information about the number of transfers completed and
pending, and the rate at which they are completing.

XDM   provides a REST API that makes these metrics available to users. To
...
where zenko-instance.net is the XDM   server's URL and us-west-video-dailies		
