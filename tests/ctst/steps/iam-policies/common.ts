...
Then('the user should receive an access denied error', function () {
    this.endForType();
    if (this.saved.ifS3Standard) {
        assert.strictEqual(this.result?.err?.includes("AccessDenied"), true);
    } else {
        assert.strictEqual(this.result?.statusCode, 403);
        assert.strictEqual(this.result?.data?.includes("AccessDenied"), true);
    }
});
